...
PendulumSystem::PendulumSystem() {
	...
	m_offsetVectors;
	int counter = 0;
	while (counter < 50*NUM_PARTICLES) {
		float fo1 = rand_uniform(-0.2f, 0.2f);
		float fo2 = rand_uniform(-0.2f, 0.2f);
		float fo3 = rand_uniform(-0.2f, 0.2f);
		m_offsetVectors.push_back(Vector3f(fo1,fo2,fo3));
		counter += 1;
	}
...
std::vector<Vector3f> PendulumSystem::evalF(std::vector<Vector3f> state)
{
	...
	Vector3f ballVelocity = state[state.size()-1];
	f.push_back(ballVelocity); //velocity of ball
	Vector3f ballForce = Vector3f(0.0f, ballGravity, 0.0f) - 0.11*ballVelocity;
	f.push_back(ballForce); //acceleration of ball
	 ...
	 for (unsigned int i=0; i<state.size()-2; i++) {
	 	...
	 	if (i % 2 == 1) {
	 		...
	 		if (distanceBetweenBallAndParticle <= (ballRadius+0.01)) {
	 			...
	 			Vector3f newForce = currentForce + ballForce/MASS;
	 			f[i] = newForce;
	 		}
	 	}
	 }
...
void PendulumSystem::draw(GLProgram& gl)
{
	...
	for (unsigned int i=0; i<m_vVecState.size()/2; i++) {
		...
		if (i==((m_vVecState.size()/2)-1)) {
			...
			int counter = 0;
			while (counter < 50) {
				Vector3f offsetVector = m_offsetVectors[(i*NUM_PARTICLES)+counter];
				...
				Vector3f newPos = position + offsetVector;
				gl.updateModelMatrix(Matrix4f::translation(newPos));
				printf("S ");
				newPos.print();
				gl.updateMaterial(Vector3f(1.0f,1.0f,1.0f),Vector3f(-1,-1,-1), Vector3f(0,0,0),1.0f,0.5f);
				drawSphere(0.05f, 8, 8);
