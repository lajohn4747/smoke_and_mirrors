...
Vector3f SmokeSystem::computeGravity(float mass){
    return -1 * mass * Vector3f(0,-9.8, 0);
}

Vector3f SmokeSystem::computeDrag(float k, Vector3f velocity){
    return -1 * -k * velocity;
}

Vector3f SmokeSystem::computeSpring(float k, float r, int i, int j, vector<Vector3f> pos) {
...
vector<Vector3f> SmokeSystem::getPositions(vector<Vector3f> state){
    vector<Vector3f> pos;
...
vector<Vector3f> SmokeSystem::getVelocities(vector<Vector3f> state){
    vector<Vector3f> vel;
...
std::vector<Vector3f> SmokeSystem::evalF(std::vector<Vector3f> state)
{
    float mass = .25;
    float r = 0.3;
    float drag = 0.1;
    ...
    for(unsigned i = 1; i < NUM_PARTICLES; ++i) {
        ...
        printf("%d %d\n", i, i+1);
        printf("\n");*/
        Vector3f forces = (computeGravity(mass) + computeDrag(drag,velocities[i]))/mass;// + springForce)/mass;
        f.push_back(forces);
...
{
    const Vector3f SMOKE_COLOR(0.73f, 0.0f, 0.83f);
    gl.updateMaterial(SMOKE_COLOR);
